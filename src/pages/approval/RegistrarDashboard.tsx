...
import { BookOpen, FileCheck, Shield, CreditCard, Clock, TrendingUp, AlertCircle, CheckCircle2, GraduationCap } from 'lucide-react';
...
const RegistrarDashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [scholarData, guideData, requestData, schoolDeanData, deanResearchData] = await Promise.all([
        ...
        approvalService.getRegistrarSchoolDeans(),
        approvalService.getRegistrarDeanResearch(),
      ]);
      ...
      const profileData = await approvalService.getRegistrarProfile();
  ...
  const sectionTitle = activeTab === 'dashboard' ? 'Registrar — Dashboard' :
    activeTab === 'students' ? 'Registrar — PhD Students' :
    activeTab === 'guides' ? 'Registrar — Guides' :
    activeTab === 'school-deans' ? 'Registrar — Dean of School' :
    activeTab === 'dean-research' ? 'Registrar — Dean of Research' :
    activeTab === 'panel-nomination' ? 'Registrar — Panel of DAC Members' :
    activeTab === 'profile' ? 'Registrar — My Profile' : 'Registrar — Dashboard';
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'profile' && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Registrar Profile</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-start gap-3 p-3 bg-gray-50/60 rounded-lg">
              ...
              <div><p className="text-xs text-gray-500 uppercase tracking-wider">Role</p><p className="font-medium text-gray-800 text-sm mt-0.5">{profile?.role || 'Registrar'}</p></div>
            </div>
            <div className="flex items-start gap-3 p-3 bg-gray-50/60 rounded-lg">
