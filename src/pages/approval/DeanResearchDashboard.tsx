...
import { UserCheck, FileCheck, Clock, TrendingUp, AlertCircle, CheckCircle2, GraduationCap, Building2 } from 'lucide-react';
...
const DeanResearchDashboard = () => {
  ...
  const [schoolDeans, setSchoolDeans] = useState<DeanRow[]>([]);
  ...
  const [filterGuideSchool, setFilterGuideSchool] = useState('');
  ...
  const [filterReqSchool, setFilterReqSchool] = useState('');
  ...
  const [filterGroupedSchool, setFilterGroupedSchool] = useState('');
  ...
  const [filterDeanName, setFilterDeanName] = useState('');
  const [filterDeanSchool, setFilterDeanSchool] = useState('');

  const validTabs = ['dashboard', 'students', 'guides', 'nomination-expert-members', 'first-dac', 'comprehensive-viva', 'colloquium', 'inch-committee', 'synopsis', 'thesis-defense', 'school-deans', 'meeting-requests'];
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [scholarData, guideData, requestData, schoolDeanResult, groupedData] = await Promise.all([
        approvalService.getScholars(role!, 0, 100),
        approvalService.getGuides(role!, 0, 100),
        approvalService.getRequests(role!, 0, 200),
        approvalService.getDeanResearchSchoolDeans().catch(() => []),
      ...
      setSchoolDeans(schoolDeanResult);
  ...
  const schools = [...new Set(scholars.map(s => s.schoolName).filter((v): v is string => Boolean(v)))];
  ...
  const guideSchools = [...new Set(guides.map(g => g.schoolName).filter((v): v is string => Boolean(v)))];
  ...
  const reqSchools = [...new Set(requests.map(r => r.schoolName).filter((v): v is string => Boolean(v)))];
  ...
  const filteredGuides = guides.filter(g => {
    ...
    return (
      ...
      (!filterGuideSchool || g.schoolName === filterGuideSchool) &&
  ...
  const getFilteredRequests = () => {
    return requests.filter(r => {
      ...
      if (filterReqSchool && r.schoolName !== filterReqSchool) return false;
        ...
        <>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h1 className="font-heading text-2xl font-bold text-gray-800">
              Welcome, <span className="text-dsu-maroon">{fullName || 'Dean of Research'}</span>
            </h1>
            <p className="text-gray-500 mt-1">University-wide PhD Research Program Overview</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-dsu-maroon/10 rounded-lg flex items-center justify-center">
                  <Building2 className="w-5 h-5 text-dsu-maroon" />
                </div>
                <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs text-gray-400">{scholars.length} total across all schools</span>
              </div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {requests.filter(r => r.status === 'PENDING_APPROVAL').slice(0, 8).map((r) => (
                  <div key={r.id} className="p-3 bg-amber-50/50 rounded-lg border border-amber-100">
                    ...
                    {r.schoolName && <p className="text-xs text-gray-400 mt-0.5">{r.schoolName}</p>}
                  </div>
                ))}
          ...
          {/* School Distribution + Recent Activity */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {schools.map((school) => {
                  const count = scholars.filter(s => s.schoolName === school).length;
                  return (
                    <div key={school} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                      <span className="text-sm text-gray-700 truncate flex-1" title={school}>{school}</span>
                      <span className="text-lg font-bold text-dsu-maroon ml-3">{count}</span>
                    </div>
                  );
                })}
                {schools.length === 0 && <p className="text-sm text-gray-400 text-center py-4">No data yet</p>}
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100">
                      <th className="text-left py-2 px-2 text-xs font-medium text-gray-500">Scholar</th>
                      <th className="text-left py-2 px-2 text-xs font-medium text-gray-500">Request</th>
                      <th className="text-left py-2 px-2 text-xs font-medium text-gray-500">Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {[...requests].sort((a, b) => (b.submittedAt || '').localeCompare(a.submittedAt || '')).slice(0, 6).map((r) => (
                      <tr key={r.id}>
                        <td className="py-2 px-2 font-medium text-gray-800 text-xs">{r.scholarName}</td>
                        <td className="py-2 px-2 text-gray-600 text-xs truncate max-w-[150px]">{getRequestTypeLabel(r.requestType)}</td>
      ...
      {activeTab === 'students' && (
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-3 bg-white rounded-xl shadow-sm border border-gray-100 p-4">
            ...
            <FilterSelect placeholder="All Schools" value={filterSchool} onChange={setFilterSchool} options={schools} />
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    ...
                    {filteredScholars.length === 0 && (
                      <tr><td colSpan={7} className="px-4 py-6 text-center text-gray-500">No scholars found</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      {activeTab === 'guides' && (
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-3 bg-white rounded-xl shadow-sm border border-gray-100 p-4">
            ...
            <FilterSelect placeholder="All Schools" value={filterGuideSchool} onChange={setFilterGuideSchool} options={guideSchools} />
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredGuides.map((g) => (
                      <tr key={g.id} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-4 py-3">{g.schoolName}</td>
                    ...
                    {filteredGuides.length === 0 && (
                      <tr><td colSpan={8} className="px-4 py-6 text-center text-gray-500">No guides found</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      {activeTab === 'school-deans' && (
        <div className="space-y-4">
          <div className="flex flex-wrap gap-3 items-center">
            <FilterInput placeholder="Search by name..." value={filterDeanName} onChange={setFilterDeanName} />
            <FilterSelect placeholder="All Schools" value={filterDeanSchool} onChange={setFilterDeanSchool} options={[...new Set(schoolDeans.map(d => d.schoolName).filter(Boolean))]} />
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {schoolDeans
                    .filter((dean) => {
                      ...
                      return (!filterDeanName || fullName.includes(filterDeanName.toLowerCase())) &&
                        (!filterDeanSchool || dean.schoolName === filterDeanSchool);
                    })
                    .map((dean) => (
          ...
          {(activeTab === 'first-dac' || activeTab === 'comprehensive-viva' || activeTab === 'colloquium') && (
            <>
              {(() => {
                ...
                const groupedSchoolOptions = [...new Set(allGrouped.map(r => r.schoolName).filter((v): v is string => Boolean(v)))];
                ...
                const filteredGrouped = allGrouped.filter(r => {
                  ...
                  if (filterGroupedSchool && r.schoolName !== filterGroupedSchool) return false;
                ...
                return (
                  <>
                    <div className="flex flex-wrap items-center gap-3 bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                      ...
                      <FilterSelect placeholder="All Schools" value={filterGroupedSchool} onChange={setFilterGroupedSchool} options={groupedSchoolOptions} />
          ...
          {!(activeTab === 'first-dac' || activeTab === 'comprehensive-viva' || activeTab === 'colloquium') && (
            <>
              <div className="flex flex-wrap items-center gap-3 bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                ...
                <FilterSelect placeholder="All Schools" value={filterReqSchool} onChange={setFilterReqSchool} options={reqSchools} />
...
export default DeanResearchDashboard;
